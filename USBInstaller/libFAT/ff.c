...
FRESULT f_unmount (FATFS *fs, const TCHAR *path)
{
	FATFS *cfs;
	int vol;
	FRESULT res = FR_OK;
	...
	if(cfs != fs) return FR_INVALID_DRIVE;
	...
	FatFs[vol] = NULL;					/* Register new fs object */
	VolToPart[vol].pd = 0;
	VolToPart[vol].pt = 0;
...
FRESULT f_sectmap (
	FIL* fp, 		/* Pointer to the file object */
	DWORD * sec,		/* Pointer to data buffer */
	UINT * cnt		/* Number of sector of buff */
)
{
	FRESULT res;
	DWORD clst; /* current cluster, total cluster cnt */
	DWORD csect; /* current sector */
	DWORD tsect, i, r, n, on;
	...
	if(!cnt || *cnt * SS(fp->fs) < fp->fsize || !sec)
		LEAVE_FF(fp->fs, FR_INVALID_PARAMETER);
	...
	if((fp->fsize & SS(fp->fs) - 1) == 0)
		tsect = fp->fsize / (SS(fp->fs));
	else
		tsect = fp->fsize / (SS(fp->fs)) + 1;

	clst = fp->sclust;
	csect = clust2sect(fp->fs, clst);
	on = 0;
	for(i = 0 ; i < tsect ; i ++) {
		n = i /  fp->fs->csize;
		r = i %  fp->fs->csize;
		if(n != on) {
			on = n;
			clst = get_fat(fp->fs, clst);	/* Follow cluster chain on the FAT */
...
/*-----------------------------------------------------------------------*/
static
FRESULT load_fs (	/* FR_OK(0): successful, !=0: any error occurred */
	BYTE pdrv,
	BYTE part,
...
)
{
	...
	fs->fs_type = 0;					/* Clear the file system object */
	fs->drv = pdrv;				/* Bind the logical drive and a physical drive */
	stat = disk_initialize(fs->drv);	/* Initialize the physical drive */
	if (stat & STA_NOINIT)				/* Check if the initialization succeeded */
		return FR_NOT_READY;			/* Failed to initialize due to no medium or hard error */
	...
	bsect = 0;
	fmt = check_fs(fs, bsect);					/* Load sector 0 and check if it is an FAT boot sector as SFD */
	if (fmt == 1 || (!fmt)) {	/* Not an FAT boot sector or forced partition number */
		UINT i;
		DWORD br[4];
		...
		i = part;						/* Partition number: 0:auto, 1-4:forced */
		if (i) i--;
							/* Find an FAT volume */
		bsect = br[i];
		fmt = bsect ? check_fs(fs, bsect) : 2;	/* Check the partition */
...
static int fs_mounted(
	...
	BYTE part          /* Part number*/
)
{
	int vol;
	FATFS *fs = NULL;
	for(vol = 0 ; vol < sizeof(FatFs)/sizeof(FATFS) ; vol ++) {
		fs = FatFs[vol];
		if(NULL != fs) {
			if(pdrv == LD2PD(vol) &&	/* Physical drive */
	...
	DWORD* vsn)
{
	...
	if(fs_mounted(pdrv, part)) {
		return FR_BUSY;
	}

	memset(vn, 0, sizeof(vn));

	memset(&tmp_fs, 0, sizeof(FATFS));
	fs = &tmp_fs;

	if((ret = load_fs(pdrv, part, fs)) != FR_OK)
		return ret;

	if (disk_read(pdrv, fs->win, fs->volbase, 1)) return FR_DISK_ERR;

	tbl = fs->win;

	if (fs->fs_type == FS_FAT32) {
		mem_cpy(vn, tbl+BS_VolLab32, 11);	/* Volume label, FAT signature */
		if(NULL != vsn) {
			*vsn = LD_DWORD(tbl+BS_VolID32);
		}
	} else {
		mem_cpy(vn, tbl+BS_VolLab, 11);	/* Volume label, FAT signature */
		if(NULL != vsn) {
			*vsn = LD_DWORD(tbl+BS_VolID);
		}
	}
	...
	DWORD vsn)
{
	...
	memset(vn, 0, sizeof(vn));
		...
		/* Create a volume label in directory form */
		vn[0] = 0;
		for (sl = 0; buf[sl]; sl++) ;				/* Get name length */
		for ( ; sl && buf[sl-1] == ' '; sl--) ;	/* Remove trailing spaces */
		if (sl) {	/* Create volume label in directory form */
			i = j = 0;
			do {
	#if _USE_LFN && _LFN_UNICODE
				w = ff_convert(ff_wtoupper(buf[i++]), 0);
	#else
				w = (BYTE)label[i++];
				if (IsDBCS1(w))
					w = (j < 10 && i < sl && IsDBCS2(label[i])) ? w << 8 | (BYTE)label[i++] : 0;
	...
	if (fs->fs_type == FS_FAT32) {
		mem_cpy(tbl+BS_VolLab32, vn, 11);	/* Volume label, FAT signature */
		if(0 != vsn) {
			ST_DWORD(tbl+BS_VolID32, vsn);
		}
	} else {
		mem_cpy(tbl+BS_VolLab, vn, 11);	/* Volume label, FAT signature */
		if(0 != vsn) {
			ST_DWORD(tbl+BS_VolID, vsn);
		}
	}
	
	return disk_write(pdrv, fs->win, fs->volbase, 1) == 0 ? FR_OK : FR_DISK_ERR;
}
...
FRESULT f_mkfs (
	...
	BYTE fmt			/* Format 0: auto, 1:FS_FAT12 2:FS_FAT16 3:FS_FAT32*/
)
{
	...
	if (fmt > 3) return FR_INVALID_PARAMETER;
	//fs = FatFs[vol];
...
FRESULT f_geometry(
	...
	DWORD * total_cyl, 
	DWORD * track_p_cyl, 
	DWORD * sector_p_track, 
	DWORD * byte_p_sector, 
	DWORD * total_sector)
{
	DSTATUS stat;
	DWORD n, sz_cyl;
	if(NULL == total_cyl || NULL == track_p_cyl || NULL == sector_p_track 
		|| NULL == byte_p_sector || NULL == total_sector) {
		return FR_INVALID_PARAMETER;
	}

	*total_cyl = *track_p_cyl = *sector_p_track = *byte_p_sector = *total_sector = 0;

	stat = disk_initialize(pdrv);
	if (stat & STA_NOINIT) return FR_NOT_READY;
	if (stat & STA_PROTECT) return FR_WRITE_PROTECTED;
	if (disk_ioctl(pdrv, GET_SECTOR_COUNT, total_sector)) return FR_DISK_ERR;
	if (disk_ioctl(pdrv, GET_SECTOR_SIZE, byte_p_sector)) return FR_DISK_ERR;

	/* Determine CHS in the table regardless of the drive geometry */
	for (n = 16; n < 256 && *total_sector / n / 63 > 1024; n *= 2) ;
	if (n == 256) n--;
	*track_p_cyl = n;
	*sector_p_track = 63;
	sz_cyl = 63 * n; // sector per cyl
	*total_cyl = *total_sector / sz_cyl;
...
FRESULT f_fdisk (
	BYTE pdrv,			/* Physical drive number */
	const DWORD szt1[],	/* Pointer to the size table for each partitions */
...
)
{
	...
	DSTATUS stat;
	DWORD sz_disk, sz_part, s_part, sz_tmp, sz_tmp1, szt[4];


	memcpy(szt, szt1, sizeof(szt));
	stat = disk_initialize(pdrv);
	if (stat & STA_NOINIT) return FR_NOT_READY;
	if (stat & STA_PROTECT) return FR_WRITE_PROTECTED;
	...
	sz_tmp1 = sz_tmp;
	if(sz_tmp > tot_cyl) return FR_INVALID_PARAMETER;
	
	/* only one entry is 0xFFFFFFFF */
	sz_tmp = 0;
	for(i = 0 ; i < 4 ; i ++ ) {
		if(szt[i] == -1) {
			sz_tmp ++;
	...
	if(sz_tmp > 1) return FR_INVALID_PARAMETER;

	/* replace -1 to remain cyl, sz_tmp1 is index of -1 slot*/
	sz_tmp = -1;
	for(i = 0 ; i < 4 ; i ++ ) {
		if(szt[i] == -1) {
			sz_tmp = i;
			szt[i] = tot_cyl - sz_tmp1;
			break;
		}
	}

	/* only last item can have -1 */
	for(i = sz_tmp1; i < 4 ; i ++) {
		if(szt[i] != 0)
			return FR_INVALID_PARAMETER;
	}

	/* final check against all zero */
	sz_tmp = 0;
	for(i = 0 ; i < 4 ; i ++ ) {
		sz_tmp += szt[i];
	}
